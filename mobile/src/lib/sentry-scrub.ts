...
//
// Installed on Sentry.init so that, regardless of `sendDefaultPii`, no
...
// See ticket #71. (Mobile is a standalone project — this intentionally mirrors
// `frontend/src/lib/sentry-scrub.ts` rather than sharing a module.)
import type { ErrorEvent } from "@sentry/react-native"

// The single, explicit Supabase auth-cookie name shared by every client
// (see the frontend `src/lib/supabase/cookie.ts`). `@supabase/ssr` splits
// large sessions into chunks named `sb-app-auth.0`, `sb-app-auth.1`, … so we
